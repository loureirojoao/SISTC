...
int main() { 
  ...
  for (n_thread = 0; n_thread < 50; n_thread++) {
    t_args[n_thread].ptr = dados+((NUM_ELEM/50)*n_thread);
    t_args[n_thread].n = (NUM_ELEM/50);
    pthread_create(&tid[n_thread], NULL, mythread, &t_args[n_thread]); 
  }
  
  for (n_thread = 0; n_thread < 50; n_thread++){
    pthread_join(tid[n_thread],NULL);
  }
